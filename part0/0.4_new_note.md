...
sequenceDiagram
    ...
    browser->>server: POST [https://studies.cs.helsinki.fi/exampleapp/new_note](https://studies.cs.helsinki.fi/exampleapp/new_note) (with text payload)
    activate server
    Note over server: Server saves the new note to its database/memory
    ...
    Note right of browser: Browser executes callback function rendering all notes (including the new one)
```
